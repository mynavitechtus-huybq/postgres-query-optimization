...
|---|---|---:|---:|---:|
| V1 | giữ nguyên, chưa tối ưu gì | 134,768 ms | 797,429 ms | — |
| V2 | thêm một index phù hợp hơn | 96,267 ms | 554,479 ms | 1,4 lần |
| V3 | viết lại câu query | 28,791 ms | 204,409 ms | 3,9 lần |
| V4 | tính sẵn kết quả để không phải tính lại | 0,040 ms | 0,038 ms | 20.985 lần |

Cột "nhanh hơn V1" tính trên bộ 5 triệu đơn.

Mình đo trên hai bộ dữ liệu vì ở bộ nhỏ khoảng cách giữa các cách chưa đủ rõ. Bộ lớn
gấp 5 lần cho thấy một điều mà bộ nhỏ không cho thấy được: **V4 là cách duy nhất không
chậm đi khi dữ liệu tăng**, vì nó luôn chỉ đọc đúng 25 dòng bất kể bảng gốc to đến đâu.
Ba cách còn lại đều chậm đi gần đúng 5 lần, vì cả ba vẫn phải đọc và cộng từng dòng.
...
[`phép đo ở quy mô 5 lần`](../plans/q3_scale_5x.txt) ·
...
## Kiểm tra lại ở quy mô gấp 5 lần

Ở bộ dữ liệu 1 triệu đơn, khoảng cách giữa các cách chưa đủ rõ để yên tâm kết luận. Nên
mình dựng thêm một database riêng gấp 5 lần — 250.000 khách và 5 triệu đơn, trong đó
1.250.965 đơn đã thanh toán — rồi đo lại đúng bốn cách trên.

Chi tiết ở [`plans/q3_scale_5x.txt`](../plans/q3_scale_5x.txt), script dựng dữ liệu ở
[`sql/03_scale_test.sql`](../sql/03_scale_test.sql).

Số trong các bảng dưới đây là trung vị của nhiều lần chạy, còn ảnh chụp là một lần chạy
cụ thể nên có thể lệch đôi chút — nhất là lần đầu, khi dữ liệu chưa nằm sẵn trong bộ nhớ
đệm.
...
| | 1 triệu đơn | 5 triệu đơn | Chậm đi bao nhiêu khi dữ liệu tăng 5 lần |
|---|---:|---:|---:|
...
Thứ nhất, **V4 là cách duy nhất không bị ảnh hưởng bởi lượng dữ liệu**. Ba cách kia đều
chậm đi gần đúng theo tỉ lệ dữ liệu tăng, vì cả ba vẫn phải đọc và cộng từng dòng. V4
thì luôn chỉ đọc 25 dòng đã tính sẵn nên thời gian giữ nguyên. Nhưng chi phí `REFRESH`
lại tăng theo đúng tỉ lệ đó, nên cái giá không biến mất mà chỉ chuyển từ lúc đọc sang
lúc cập nhật.

![Chi phí REFRESH ở quy mô 5 lần](../images/q3-5x-refresh.png)

Thứ hai, **thứ hạng giữa các cách không đổi**. V3 vẫn nhanh hơn V2 khoảng 2,7 lần ở bộ
lớn, so với 3,3 lần ở bộ nhỏ. Kết luận chọn V3 làm phương án chính không phụ thuộc vào
quy mô dữ liệu.

Kết quả cũng vẫn đúng ở quy mô lớn: 25 dòng, không dòng nào lệch, tổng doanh thu
637.603.432,21 giống nhau giữa cả ba cách. Và V4 vẫn lệch toàn bộ 50 dòng khi chạy ở
múi giờ Việt Nam, đúng như ở bộ nhỏ.
...
Mình chọn **V3 làm phương án chính**. Nó nhanh hơn 3,9 lần ở bộ 5 triệu đơn, không thêm
bất cứ thứ gì phải vận hành, dữ liệu luôn mới nhất, chạy ở múi giờ nào cũng đúng, và tất
cả những gì cần làm chỉ là viết lại câu query cộng với index đã có từ V2.
...
Bản đo gốc của lần đó được giữ lại nguyên vẹn:
...
Ba file trên đo ở trạng thái bảng khác với V1 đến V4 (bảng chưa chạy `VACUUM FULL`, nên
baseline là 153,4 ms thay vì 134,8 ms), nên chỉ đọc để tham khảo cách làm chứ không so
trực tiếp từng con số với các cách mới được. Phần đầu mỗi file có ghi rõ khác biệt.
...
Biến kết quả của hàm thành một cột thật, để Postgres tự thu thập thống kê cho nó như mọi
cột khác. Cách này cũng sửa được nguyên nhân thứ nhất mà không cần `CREATE STATISTICS`.

Bỏ vì cái giá quá đắt: lệnh `ALTER TABLE` viết lại toàn bộ bảng mất 2,2 giây và giữ khoá
`ACCESS EXCLUSIVE` suốt thời gian đó, nghĩa là hệ thống thật sẽ chết trong 2,2 giây.
Bảng phình từ 67 lên 75 MB và không đòi lại được nếu sau này bỏ cột. Đổi lại thì vẫn
chậm hơn V3, và cũng chốt cứng múi giờ UTC nên gặp đúng vấn đề như V4.

Số liệu đầy đủ ở [`plans/q3_alternatives.txt`](../plans/q3_alternatives.txt).
